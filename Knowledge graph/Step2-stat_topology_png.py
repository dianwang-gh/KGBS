...
def load_graph(filepath='PAEs_network.gpickle', tpr_filepath='TPR_number.csv'):
    ...
    # Add edge attributes (including CooccurrenceProbability)
    for _, row in tpr_df.iterrows():
        ...
        if G.has_edge(u, v):
            G.edges[u, v].update({
                'mechanism': row['Name'],
                'number': row['Number'],
                'cooccurrence_prob': row['CooccurrenceProbability']   # 新增
            })
...
# ------------------------ Edge Width Normalization Function (using cooccurrence_prob) ------------------------
def normalize_edge_width_by_prob(G, min_width=1.0, max_width=10.0):

    valid_edges = [(u, v, data) for u, v, data in G.edges(data=True) if 'cooccurrence_prob' in data]
    probs = [data['cooccurrence_prob'] for u, v, data in valid_edges]
    ...
    for u, v, data in G.edges(data=True):
        if 'cooccurrence_prob' in data:
            p = data['cooccurrence_prob']
            width = ((p - min_prob) / (max_prob - min_prob)) * (max_width - min_width) + min_width
        else:
            width = min_width
...
def statistics_by_edge_type(G):
    stats = {}
    for u, v in G.edges():
        mech = G.edges[u, v].get('mechanism', 'Unknown')
        rel_type = G.edges[u, v].get('relation_type', 'unknown')
        if mech not in stats:
            stats[mech] = {'Sequential': 0, 'V-Shape': 0, 'Paternal': 0}
        if rel_type == 'sequential':
            stats[mech]['Sequential'] += 1
        elif rel_type == 'v_shape':
            stats[mech]['V-Shape'] += 1
        elif rel_type == 'paternal':
            stats[mech]['Paternal'] += 1
    stats_df = pd.DataFrame.from_dict(stats, orient='index')
    stats_df.index.name = 'Mechanism'
    stats_df.reset_index(inplace=True)
    stats_df.to_csv('relation_statistics_by_mechanism_edge_based.csv', index=False)
    print("Saved edge-based statistics to 'relation_statistics_by_mechanism_edge_based.csv'.")
...
# ------------------------ Export Edge Data (using existing cooccurrence_prob) ------------------------
def export_edges_data(G, filepath='edges_info.csv'):
    ...
    for u, v in G.edges():
        edges_data.append({
            ...
            'CooccurrenceProbability': G.edges[u, v].get('cooccurrence_prob', 0),  # 直接使用现有值
...
# ------------------------ Draw and Save Topology Graph (using cooccurrence_prob based widths) ------------------------
def draw_topology_graph(G, output_file='topology_graph.png'):
    ...
    # Edge settings: use normalized widths based on cooccurrence_prob
    edge_widths = normalize_edge_width_by_prob(G)
    ...
    plt.legend(handles=legend_elements, loc='upper right', fontsize=30, frameon=False)
    plt.axis('off')
    plt.tight_layout()
