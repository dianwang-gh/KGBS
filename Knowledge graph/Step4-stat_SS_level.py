    ...
    for _, row in tpr_df.iterrows():
        ...
        if G.has_edge(u, v):
            G.edges[u, v].update({
                'mechanism': row['Name'],
                'number': row['Number'],
                'cooccurrence_prob': row['CooccurrenceProbability']  
            })
...
# ------------------------ Edge Width Normalization Function (based on cooccurrence_prob) ------------------------
...
# ------------------------ Export Edge Data (directly using stored cooccurrence_prob) ------------------------
def export_edges_data(G, filepath='edges_info_new.csv'):
    """Export edge relationships and co-occurrence probability (using stored value)"""
    edges_data = []
    for u, v in G.edges():
        edges_data.append({
            'Source': u,
            ...
            'CooccurrenceProbability': G.edges[u, v].get('cooccurrence_prob', 0),  
